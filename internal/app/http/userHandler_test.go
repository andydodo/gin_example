...
func TestAppServer_RegisterUserHandler(t *testing.T) {
	...
	mockService.CreateUserFn = func(u *types.User, password string) (*types.User, error) {
		if len(password) < 8 {
...
func TestAppServer_LoginUserHandler(t *testing.T) {
	mockService := mock.UserService{}
	mockService.LoginFn = func(email string, password string) (*types.User, error) {
		if password != "password" {
			return nil, errors.New("bad login")
		}
		return &types.User{Model: gorm.Model{ID: 1}}, nil
	}
	appServer := AppServer{UserService: &mockService, Logger: log.New(os.Stdout, "", 0)}
...
func TestAppServer_GetUserHandler(t *testing.T) {
	mockService := mock.UserService{}
	mockService.GetUserFn = func(id string) (*types.User, error) {
		if id != "userId" {
...
func TestAppServer_GetMeHandler(t *testing.T) {
	...
	mockService.GetUserFn = func(id string) (*types.User, error) {
		...
		return &types.User{Model: gorm.Model{ID: 1}, Email: "heisenberg@gmail.com", Name: "heisenberg"}, nil
	}
	appServer := AppServer{UserService: &mockService, Logger: log.New(os.Stdout, "", 0)}
